...
//   formData.append('upload_preset', 'LocalBharat');
//   formData.append('api_key', '632877426687691'); // Get from Cloudinary dashboard

//   try {
//     // Using fetch instead of axios for better FormData handling
//  const response = await axios.post(`https://api.cloudinary.com/v1_1/dqncbfwfn/image/upload`, formData)
//     if (!response) {
...
const uploadImageToCloudinary = async (file) => {
  ...
  formData.append('file', file);
  formData.append('upload_preset', 'LocalBharat');
  // Remove the api_key from formData - it should not be sent client-side
  // formData.append('api_key', '632877426687691'); // This is insecure!

  try {
    const response = await axios.post(
      `https://api.cloudinary.com/v1_1/dqncbfwfn/image/upload`,
      formData,
      {
        headers: {
...
const handleImageUpload = async (e) => {
  ...
  try {
    // Upload files sequentially (better than Promise.all for error handling)
    ...
    for (const file of files) {
      try {
        ...
        if (file.size > 10 * 1024 * 1024) { // 10MB limit
          toast.warning(`Skipped ${file.name}: File too large (max 10MB)`);
          continue;
        }
  ...
  const removeImage = async (index) => {
    ...
    // If it's a new image (not existing), you might want to delete it from Cloudinary
    // Note: Cloudinary doesn't require you to delete images unless you want to
    // If you want to delete, you would need to implement a server-side endpoint
    // to handle the deletion using Cloudinary's admin API
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      const productData = {
        ...
        images: previewImages.map(img => img.url) // Use Cloudinary URLs
      };
